...
def store_celldata(fn, curs):
    ...
    min_lon, max_lon = 19.8, 20.0
    min_lat, max_lat = 47.9, 48.16 # ezek alkotják a lehatárolás keretét
...
def read_netcdf(nc_file, ds_name, cell_id, cur):
    ...
    try:
        ...
        print(f" - {oi}{si} cella adatait töltöm...")    
        for t in range(len(time)):
            # a meterológiai adat
            value = dataset.variables[ds_name][t]
            if ds_name == 'pr':
                data = (value[si][oi]) * 10800   # X mm/s -> 3 óra alatt 3*60*60 (10800 * X mm)
            elif ds_name == 'tas':
        ...
        row = cur.fetchone()[0]
        print(f"    * ... eddig {row} sort szúrtam be a(z) {ds_name} táblába...")
