...
"""
 Menti az eredményeket az adatbázisba (pr_disaggregated tábla).
"""
def save_to_database(result_df, db_path='data/basin.db'):
    print(f"\n=== Adatbázisba mentés ===")
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS pr_disaggregated (
            time_hourly TEXT NOT NULL,
            cell_id INTEGER NOT NULL,
            pr_hourly_disaggregated REAL NOT NULL,
            PRIMARY KEY (time_hourly, cell_id)
        )
    ''')
    
    # Adatok előkészítése
    db_data = result_df[['time_hourly', 'cell_id', 'pr_hourly_disaggregated']].copy()
    db_data['time_hourly'] = db_data['time_hourly'].dt.strftime('%Y-%m-%d %H:%M')
    
    # Korábbi adatok törlése (ugyanaz időszak + cellák)
    cell_ids = db_data['cell_id'].unique()
    time_range = (db_data['time_hourly'].min(), db_data['time_hourly'].max())
    
    placeholders = ','.join(['?'] * len(cell_ids))
    cursor.execute(f'''
        DELETE FROM pr_disaggregated 
        WHERE cell_id IN ({placeholders}) 
        AND time_hourly BETWEEN ? AND ?
    ''', list(cell_ids) + [time_range[0], time_range[1]])
    
    deleted_rows = cursor.rowcount
    print(f"  Korábbi rekordok törölve: {deleted_rows}")
    
    # Új adatok beszúrása batch-ben (hibakezelés)
    try:
        db_data.to_sql('pr_disaggregated', conn, if_exists='append', index=False)
        new_rows = len(db_data)
    except Exception as e:
        # Ha UNIQUE constraint hiba, egyenként próbáljuk
        if "UNIQUE constraint" in str(e):
            print(f"  ⚠️ UNIQUE constraint - egyenkénti beszúrás...")
            new_rows = 0
            for _, row in db_data.iterrows():
                try:
                    cursor.execute('''
                        INSERT OR REPLACE INTO pr_disaggregated 
                        (time_hourly, cell_id, pr_hourly_disaggregated) 
                        VALUES (?, ?, ?)
                    ''', (row['time_hourly'], row['cell_id'], row['pr_hourly_disaggregated']))
                    new_rows += 1
    ...
    # Ellenőrzés
    cursor.execute('SELECT COUNT(*) FROM pr_disaggregated')
    total_rows = cursor.fetchone()[0]
    
    conn.commit()
    conn.close()
    
    print(f"  ✅ {new_rows} rekord mentve")
    print(f"  📊 Összes rekord a táblában: {total_rows}")
    
    return True

"""
 DSS fájl generálása cellánként, 10 éves periódusokban.
 A petcalc_01.py mintájára a hecdss könyvtárat használja.
"""
def create_dss_files(result_df, output_dir='results'):
    print(f"\n=== DSS fájlok generálása ===")
    ...
    try:
        ...
        print("   Telepítse: pip install hecdss")
        return []
    
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Cellák és évek meghatározása
    cells = sorted(result_df['cell_id'].unique())
    result_df = result_df.copy()
    result_df['year'] = result_df['time_hourly'].dt.year
    years = sorted(result_df['year'].unique())
    
    # 10 éves periódusok meghatározása
    decade_start = (years[0] // 10) * 10
    decades = []
    current_start = decade_start
    while current_start <= years[-1]:
        decade_end = min(current_start + 9, years[-1])
        decades.append((current_start, decade_end))
        current_start += 10
    
    print(f"  Cellák: {cells}")
    print(f"  Évek: {years[0]}-{years[-1]}")
    print(f"  10 éves periódusok: {decades}")
    
    dss_files_created = []
    
    for decade_start, decade_end in decades:
        decade_data = result_df[
            (result_df['year'] >= decade_start) & 
            (result_df['year'] <= decade_end)
        ].copy()
        ...
        # DSS fájl neve
        dss_filename = output_dir / f'pr_disaggregated_{decade_start}_{decade_end}.dss'
        
        try:
            # HEC-DSS fájl létrehozása
            dss = HecDss(str(dss_filename))
            
            for cell_id in cells:
                cell_data = decade_data[decade_data['cell_id'] == cell_id].copy()
                
                if len(cell_data) == 0:
                    continue
                
                # Idősor rendezése
                cell_data = cell_data.sort_values('time_hourly')
                
                # DSS path (HEC 7.0 formátum)
                # A=basin, B=location, C=parameter, D=date, E=interval, F=version  
                pathname = f"/BASIN/CELL_{cell_id}/PRECIPITATION//1HOUR/FORECAST/"
                
                print(f"    📝 {cell_id} cella ({len(cell_data)} óra) -> {dss_filename.name}")
                print(f"       Pathname: {pathname}")
                
                # RegularTimeSeries objektum létrehozása
                tsc = RegularTimeSeries()
                tsc.id = pathname
                tsc.values = cell_data['pr_hourly_disaggregated'].values.astype(float)
                # FutureWarning javítása - numpy array explicit használat
                import numpy as np
                tsc.times = np.array(cell_data['time_hourly'].dt.to_pydatetime()).tolist()
                tsc.units = "MM"
                ...
                print(f"    ✅ {cell_id} cella mentve")
            
            dss.close()
            dss_files_created.append(dss_filename)
            print(f"    🎯 DSS fájl kész: {dss_filename.name}")
            
        except Exception as e:
            print(f"    ❌ Hiba a DSS fájl létrehozásakor ({dss_filename.name}): {e}")
    
    print(f"  � Összesen {len(dss_files_created)} DSS fájl létrehozva")
    return dss_files_created

"""
 Menti az eredményeket CSV és SQLite formátumban.
"""
def save_results(result_df, cella_id, output_dir='results', db_path='data/basin.db', create_dss=True):
    ...
    # ÚJ: Adatbázisba mentés
    try:
        save_to_database(result_df, db_path)
    except Exception as e:
        print(f"❌ Adatbázisba mentési hiba: {e}")
    
    # ÚJ: DSS fájlok létrehozása (opcionális)
    if create_dss:
        try:
            dss_files = create_dss_files(result_df, output_dir)
        except Exception as e:
            print(f"❌ DSS fájl létrehozási hiba: {e}")
            print("   (Lehet, hogy a pyhecdss könyvtár nincs telepítve)")
...
def main():
    ...
    parser.add_argument('--no-dss',
                       action='store_true',
                       help='DSS fájlok létrehozásának kihagyása')
    parser.add_argument('--output-dir',
                       default='results',
                       help='Kimeneti könyvtár (CSV és DSS fájlokhoz)')
    ...
    csv_file = save_results(
        result_df, 
        args.cell_id, 
        output_dir=args.output_dir,
        db_path=args.db_path,
        create_dss=not args.no_dss
    )
    
    print(f"\n🎯 KÉSZ!")
    print(f"   A véletlenszerű disaggregáció elkészült!")
    print(f"   Az órás csapadékadatok helye: {csv_file}")
    print("="*50)
